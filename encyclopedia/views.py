...
# Takes the name from the url string, invokes the Entry.html format, and requests the specific entry from a python dict returned by util.get_entry.
...
def search(request):
# This function supports the search feature.  It determines if an exact match exists via an "ExactMatch" value in util.get_entry.
    ...
    # If client search query is not an exact match, results.html is rendered with a list of substring match results.
        results = []
...
def create(request):
# This function just catches a redirect from urls.py and renders the create.html page.
...
# Calls util.overwrite_entry to save the client's updated data from edit.html over the existing file.
# Next, it calls util.get_entry to serve the newly-updated entry content to the Entry.html template.
...
# Selects a random entry from util.list_entries and serves the content to the entry.html template.
